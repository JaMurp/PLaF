...
let rec level t i =
   ...
   |Node(node, left, right) ->
      if i = 1 then 
         ...
         (level left (i -1)) @ (level right (i -1))
...
let rec level_helper t i =
   if i = 0 then 
      []
   else 
      level_helper t (i - 1) @ [level t i] 

let levels t =
   level_helper t (height t)
...
let rec paths_to_leaves t =
   match t with
   | Empty ->  []  (* For an empty tree, there are no paths. *)
   | Node(d, Empty, Empty) ->  [[d]]  (* For a leaf node, the path is just the node itself. *)
   | Node(d, lt, rt) ->
       let left_paths = (if lt = Empty then [] else paths_to_leaves lt)
       and right_paths = (if rt = Empty then [] else paths_to_leaves rt) in
       List.map (fun path -> d :: path) (left_paths @ right_paths)  (* Prepend current node to all paths. *)
                          *)
